...
func TestAccAzureADApplicationCertificate_basic(t *testing.T) {
	resourceName := "azuread_application_certificate.tests"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ExpectError: acceptance.RequiresImportError("azuread_application_certificate"),
			},
		},
	})
}
...
resource "azuread_application_certificate" "tests" {
...
resource "azuread_application_certificate" "import" {
  application_object_id = azuread_application_certificate.tests.application_object_id
  key_id                = azuread_application_certificate.tests.key_id
  type                  = azuread_application_certificate.tests.type
  end_date              = azuread_application_certificate.tests.end_date
  value                 = azuread_application_certificate.tests.value
}
`, template)
}
